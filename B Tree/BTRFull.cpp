...
public:
	...
	void printAllKeys() { printAllKeys(root); }
	void printAllKeys(BNode<T> *p);
	void printNode(BNode<T> *p);
	void printBetween(T minKey, T maxKey); // prints keys between requested values
	void printBetween(T minKey, T maxKey, BNode<T>* t); // prints keys between requested values
	T* search(T x) { return search(root, x); } //Search for specific node on the tree
...
T* BTree<T>::search(BNode<T> *p, T x)
{
	...
	//We only increase the index value in case he didn`t get to the number of keys and they key on that place is less than the value that we are looking for
	while (i < (*p)->nkeys && (*p)->Key[i] < x)
	{
		i++;
	}
	
	if ((*p)->Key[i] == x)//We found it
		return p;
	if (isLeaf)//If we got to a leaf and the value is not there , so he is not on the tree
		return NULL;
	search((*p)->Son[i]);//When the value is bigger than the values of the current node

}


template <typename T>
void BTree<T>::printAllKeys(BNode<T> *p)
{
}

template<typename T>
void BTree<T>::printNode(BNode<T>* p)//Printing all the values of a node
{
...
{
	if (t) {//Checking that the node exsists
...
template <typename T>
void BTree<T>::printSubTree(BNode<T> *p)
{
	if (p == NULL) return;//Nothing to print
	//Checking f it`s leaf
	bool isLeaf = true;
	for (int i=0;i<M;i++)
		if (p->Son[i] != NULL)
		{
	...
	//When we get to a leaf , we print it
	if (isLeaf)
		printNode(p);

	for (int i = 0; i < (p)->nkeys; i++)
	{
		if (p->Son[i] != NULL) {//Checking if the son has minor values , because then we need to print it first
			if (((p)->Son[i])->Key[0] < (p)->Key[i])
			{
				printSubTree((p)->Son[i]);
			}
			//Printing the father`s value
			cout << (p)->Key[i];
		}
	}
	if (!isLeaf)//If it`s not leaf , continue we recursive call
	printSubTree((p)->Son[(p)->nkeys]);
	//return;
}
...
template <typename T>
void BTree<T>::printBetween(T minKey, T maxKey) // prints the keys in the requested range
{
	printBetween(minKey, maxKey, root);

}
template <typename T>
void BTree<T>::printBetween(T minKey, T maxKey, BNode<T>* t) // prints the keys in the requested range
{
	if (t == NULL)
		return;
	int i;
	for (i = 0; i < t->nsons - 1; i++)
	{
		printBetween(minKey, maxKey, t->Son[i]);
		if (t->Key[i] >= minKey && t->Key[i] <= maxKey)
			cout << t->Key[i];
	}
	printBetween(minKey, maxKey, t->Son[i]);
}
